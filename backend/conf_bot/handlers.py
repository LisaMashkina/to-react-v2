...
text = 'https://t.me/pum_conf_bot?start=conf_reg_approve'


async def approve_registartion(user_id):
    pass
    

@router.message(CommandStart(deep_link=True, magic=F.args == 'conf_reg_approve'))
async def cmd_start(message: Message, command: Command):
    await message.answer(text='Ваш аккаунт был успешно верифицирован! Выберите, хотите ли вы вернуться на сайт и продолжить регистрацию там, или же хотите продолжить здесь?', reply_markup=kb.where_reg)
    await approve_registartion(message.from_user)
    

@router.callback_query(F.data == 'start_reg')
@router.message(CommandStart())
async def cmd_start(ccallbcak: CallbackQuery,message: Message):
...
async def confirm_order(callback: CallbackQuery, callback_data: Cake, state: FSMContext):
    ...
    try:
        ...
        await bot.edit_message_text(text=f'Торт с такими ингридиентами невозможен. Попробуйте ещё раз.', chat_id=callback.message.chat.id, message_id=callback.message.message_id, reply_markup=kb.kb_for_approve())
    finally:
    	print('inserted')
